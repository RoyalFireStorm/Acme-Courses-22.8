...
public interface AnyRoleRepository extends AbstractRepository{
		...
		@Query("select a from UserAccount a join a.roles r where a.enabled = true and (type(r) = Learner or type(r) = Teacher) and Administrator not in (select type(r) from UserAccount u join u.roles r where u.id = a.id)")
